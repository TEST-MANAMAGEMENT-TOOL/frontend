...
const systemFields = [
  ...
  { value: 'mainFeature', label: 'Main Feature' },
  { value: 'featureSubsection', label: 'Feature Subsection' },
...
export const RequirementImportModal: React.FC<RequirementImportModalProps> = ({
  isOpen,
  onClose,
  testPlanId,
  onImportComplete,
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      if (!localTestPlanId) {
        setError('Test Plan ID is required');
        setIsImporting(false);
        return;
      }
      ...
      console.log('Making request to:', '/api/importrequirements');
      
      const response = await fetch(
        '/api/importrequirements',
        {
          method: 'POST',
          headers: {
      ...
      // Check if response is a created requirement object (has reqId or description and no success field)
      if ((result.reqId || result.description) && result.success === undefined) {
        // Backend returned the created requirement, treat as success
        importResult = {
          success: true,
          message: 'Requirement imported successfully',
          importedCount: 1,
          failedCount: 0,
        };
            ...
            <div className="space-y-4">
              ...
              <div className="flex justify-end gap-2">
                ...
                <Button onClick={handleImport} disabled={isImporting}>
                  {isImporting ? 'Importing...' : 'Import Requirements'}
                </Button>
              </div>
            </div>
          )}
